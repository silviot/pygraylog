...
def test_obj_creation():
    ...
    api = graylogapi.GraylogAPI(
        "https://test.com/foo/bar/", username="Zack", password="Zack", api_key=None
    )
    ...
    api = graylogapi.GraylogAPI(
        "https://test.com/foo/bar/", username=None, password=None, api_key="ABCDEFG"
    )
    assert api._path == "/foo/bar/"
...
def test_auth_header():
    ...
    # test based on api-key token access level
    api = graylogapi.GraylogAPI("/", username=None, password=None, api_key="ABCDEFG")
    header = api.build_auth_header()
